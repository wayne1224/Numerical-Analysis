...
void wFile(vector<double> offD , vector<matrix> As, int times){
    FILE *fp = fopen("002.txt" , "w");

    if(fp != NULL){
        for(int i = 0 ; i < times ; i++){
            fprintf(fp , "times =  %d\n", i);
            fprintf(fp , "eigenvalue:\n");

            for(int j = 0 ; j < As[i].row_size ; j++){
                for(int k = 0 ; k < As[i].column_size ; k++){
                    fprintf(fp , "%9.5f " , As[i].data[j][k]);
                }
